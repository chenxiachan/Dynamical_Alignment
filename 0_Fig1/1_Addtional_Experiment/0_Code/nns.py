...
class Net(nn.Module):
    def __init__(self, config, encoding, is_umap_preprocessed=False):
        super().__init__()
        self.encoding = encoding
        self.num_steps = config.num_steps

        if is_umap_preprocessed:
            if encoding in ['lorenz', 'chen', 'rossler', 'aizawa', 'nose_hoover', 'sprott', 'chua','mixed_oscillator']:
                self.input_dim = config.n_components * 3
            else: # For other encodings after UMAP
                self.input_dim = config.n_components
        elif encoding in ['lorenz', 'chen', 'rossler', 'aizawa', 'nose_hoover', 'sprott', 'chua','mixed_oscillator']:
            self.input_dim = config.n_components * 3
        elif encoding == 'default':
        ...
        print(f"Initializing SNN with encoding '{encoding}', UMAP preprocessed: {is_umap_preprocessed}, Input size: {self.input_dim}")
