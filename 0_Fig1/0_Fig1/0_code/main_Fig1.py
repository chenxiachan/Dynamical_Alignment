...
def load_data_with_encoding(config, apply_umap=False, n_components=5, encoding='default', subsample_size=0.1):
    ...
    if encoding == 'umap'or apply_umap or encoding in ['lorenz']:
        all_data = torch.cat([train_dataset.data.float(), test_dataset.data.float()]).view(-1, 28 * 28)

        print("Applying UMAP for encoding...")
        ...
        apply_umap = False  # Avoid duplicate processing
...
def main_statistical():
    config = Config()
    save_path = 'statistical_results_1.0'
    os.makedirs(save_path, exist_ok=True)

    experiments = [
        {'name': 'Default-SNN', 'model_type': ['SNN'], 'encoding': 'default'},
        {'name': 'Lorenz-SNN', 'model_type': ['SNN'], 'encoding': 'lorenz'},
        {'name': 'Rate-SNN', 'model_type': ['SNN'], 'encoding': 'rate'},
        {'name': 'Phase-SNN', 'model_type': ['SNN'], 'encoding': 'phase'},
        {'name': 'Latency-SNN', 'model_type': ['SNN'], 'encoding': 'latency'},
        {'name': 'TTFS-SNN', 'model_type': ['SNN'], 'encoding': 'ttfs'},
        {'name': 'Delta-SNN', 'model_type': ['SNN'], 'encoding': 'delta'},
        {'name': 'Burst-SNN', 'model_type': ['SNN'], 'encoding': 'burst'},

        {'name': 'Lorenz-ANN', 'model_type': ['ANN'], 'encoding': 'lorenz'},
        {'name': 'Umap-ANN', 'model_type': ['ANN'], 'encoding': 'umap'},
        {'name': 'Default-ANN', 'model_type': ['ANN'], 'encoding': 'default'}
    ]
    num_runs = 10  # Reduced for testing, set back to 20 for production
    subsample_size = 1.0 # Using 20% for faster iteration, increase for production
    ...
    for run in range(num_runs):
        ...
        for experiment in experiments:
            ...
            train_loader, test_loader, encoding_time = load_data_with_encoding(
                config,
                apply_umap=(experiment['encoding'] == 'lorenz'),
