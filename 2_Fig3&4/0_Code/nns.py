...
class ANNNet(nn.Module):
    def __init__(self, config, encoding):
        ...
        # self.bn1 = nn.BatchNorm1d(config.num_hidden)
        # self.bn2 = nn.BatchNorm1d(config.num_hidden)
        # self.bn3 = nn.BatchNorm1d(config.num_hidden)
    ...
    def forward(self, x):
        if self.encoding == 'lorenz':
            if len(x.shape) > 2:  
                x = x[:, -1, :]  
        elif self.encoding == 'optimal_contrast':
            if len(x.shape) > 2:  
                x = x.mean(dim=1) 
        x = x.view(x.size(0), -1)
...
class Net(nn.Module):
    def __init__(self, config, encoding):
        ...
        if encoding in ['lorenz', 'chen', 'rossler', 'aizawa', 'nose_hoover', 'sprott', 'chua']:
